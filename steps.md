...
- [x] **36.** LinkedIn launch post. Lead with a generated SVG or opaque PNG (use the tool to make it), one-paragraph pitch (LLM-first spec → blueprint SVG, no design tool), link to GitHub. Verify: post is live; the embedded image was produced by the tool itself.
